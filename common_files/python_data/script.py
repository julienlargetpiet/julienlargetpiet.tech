...
data["CA_PAR_PRODUIT"] = data.groupby("PRODUCTLINE")["REVENUE"].transform("sum")
...
data_top_ventes = (
    data.groupby("PRODUCTLINE")["REVENUE"]
    .sum()
...
df_top.columns = ["PRODUCTLINE", "REVENUE"] # because of convertion series -> dataframe, we do that to be defensive
...
ca_mensuel = data.groupby("MONTH_ID")["REVENUE"].sum()
...
df_ca_mensuel.columns = ["MONTH_ID", "REVENUE"]
...
data_dealsize = data.groupby("DEALSIZE")["REVENUE"].apply(list)
...
ca_detailed = data.groupby(["COUNTRY", "DEALSIZE"])["REVENUE"].sum()
